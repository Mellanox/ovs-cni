...
# --- Tool paths ---
TOOL_BIN_DIR       ?= ./bin
ADDLICENSE         ?= $(TOOL_BIN_DIR)/addlicense
ADDLICENSE_VERSION ?= latest
GO_LICENSES        ?= $(TOOL_BIN_DIR)/go-licenses
...
.PHONY: addlicense
addlicense: $(TOOL_BIN_DIR)
	@if [ ! -f "$(ADDLICENSE)" ]; then \
		echo "Installing addlicense to $(ADDLICENSE)..."; \
		GOROOT="" GOBIN=$(abspath $(TOOL_BIN_DIR)) go install github.com/google/addlicense@$(ADDLICENSE_VERSION); \
	else \
...
copyright-check: addlicense
	...
	@GOROOT="" git ls-files '*' $(GIT_LS_FILES_EXCLUDES) | xargs grep -ILi "$(COPYRIGHT_OWNER)" | xargs -r $(ADDLICENSE) -check -c "$(COPYRIGHT_OWNER)" -l $(COPYRIGHT_STYLE) $(COPYRIGHT_FLAGS) -y $(COPYRIGHT_YEAR)

# Fix headers
.PHONY: copyright
copyright: addlicense
	@echo "Adding copyright headers..."
	@GOROOT="" git ls-files '*' $(GIT_LS_FILES_EXCLUDES) | xargs grep -ILi "$(COPYRIGHT_OWNER)" | xargs -r $(ADDLICENSE) -c "$(COPYRIGHT_OWNER)" -l $(COPYRIGHT_STYLE) $(COPYRIGHT_FLAGS) -y $(COPYRIGHT_YEAR)

# Install go-licenses tool locally
.PHONY: go-licenses
go-licenses: $(TOOL_BIN_DIR)
	@if [ ! -f "$(GO_LICENSES)" ]; then \
		echo "Installing go-licenses to $(GO_LICENSES)..."; \
		GOROOT="" GOBIN=$(abspath $(TOOL_BIN_DIR)) go install github.com/google/go-licenses@$(GO_LICENSES_VERSION); \
	else \
