...
var _ = Describe("run", func() {
	...
	It("returns 1 when --cni-bin-dir is a relative path", func() {
		src := writeTempFile(tmpDir, "ovs", []byte("bin"), 0o755)
		os.Args = []string{"entrypoint", "--cni-bin-dir=relative/path", "--ovs-bin-file=" + src, "--ovs-mirror-producer-bin-file=" + src, "--ovs-mirror-consumer-bin-file=" + src}
	...
	It("returns 1 when --ovs-bin-file does not exist", func() {
		os.Args = []string{"entrypoint",
			"--cni-bin-dir=" + tmpDir,
			"--ovs-bin-file=" + filepath.Join(tmpDir, "no-such-binary"),
			"--ovs-mirror-producer-bin-file=" + filepath.Join(tmpDir, "no-such-binary"),
			"--ovs-mirror-consumer-bin-file=" + filepath.Join(tmpDir, "no-such-binary"),
		}
		Expect(run()).To(Equal(1))
	})
	...
	It("copies the binary to --cni-bin-dir and exits 0 on SIGTERM", func() {
		...
		destFile := filepath.Join(destDir, "ovs")
		destMirrorProducerFile := filepath.Join(destDir, "ovs-mirror-producer")
		destMirrorConsumerFile := filepath.Join(destDir, "ovs-mirror-consumer")
		Eventually(destFile, 5*time.Second, 50*time.Millisecond).Should(BeAnExistingFile())
		Eventually(destMirrorProducerFile, 5*time.Second, 50*time.Millisecond).Should(BeAnExistingFile())
		Eventually(destMirrorConsumerFile, 5*time.Second, 50*time.Millisecond).Should(BeAnExistingFile())
